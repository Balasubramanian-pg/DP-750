...
```
-- What's our current pipeline by region? SELECT accounts.region AS Region, SUM(opportunity.amount) AS Pipeline FROM opportunity JOIN accounts ON opportunity.accountid = accounts.id WHERE opportunity.forecastcategory = 'Pipeline' AND opportunity.stagename NOT LIKE '%closed%' GROUP BY accounts.region;
```
...
- Previous: [[08 - Implement a foreign catalog]]
- Next: [[10 - Exercise]]
