...
```
CREATE CONNECTION postgresql_prod TYPE postgresql OPTIONS ( host 'prod-db.example.com', port '5432', user secret ('prod-secrets','postgres-user'), password secret ('prod-secrets','postgres-password') );
```
...
```
CREATE CONNECTION azuresql_prod TYPE SQLSERVER OPTIONS ( host 'myserver.database.windows.net', port '1433', user secret ('prod-secrets','azuresql-user'), password secret ('prod-secrets','azuresql-password') );
```
...
```
CREATE FOREIGN CATALOG IF NOT EXISTS prod_customer_data USING CONNECTION postgresql_prod OPTIONS (database 'customers');
```
...
```
SELECT * FROM prod_customer_data.public.transactions
```
...
```
-- Refresh an entire catalog REFRESH FOREIGN CATALOG prod_customer_data; -- Refresh a specific schema REFRESH FOREIGN SCHEMA prod_customer_data.public; -- Refresh a specific table REFRESH FOREIGN TABLE prod_customer_data.public.transactions;
```
...
```
GRANT USE CATALOG ON CATALOG prod_customer_data TO `data-analysts`; GRANT USE SCHEMA ON SCHEMA prod_customer_data.public TO `data-analysts`; GRANT SELECT ON TABLE prod_customer_data.public.transactions TO `data-analysts`;
```
...
```
SELECT customer_id, SUM(transaction_amount) AS total_spent FROM prod_customer_data.public.transactions WHERE transaction_date >= '2024-01-01' GROUP BY customer_id ORDER BY total_spent DESC LIMIT 100;
```
...
- Previous: [[07 - Implement DDL operations]]
- Next: [[09 - Configure AI-BI Genie instructions]]
