...
```
sales_data (Catalog) └── bronze (Schema – raw data) ├── raw_orders (Table) └── raw_customers (Table) └── silver (Schema – cleaned / validated) ├── cleaned_orders (Table) └── validated_customers (Table) └── gold (Schema – aggregated / business-level) ├── sales_summary (Table) ├── customer_lifetime_value (Table) └── vw_customer_stats (View example: prefix views with vw_)
```
...
```
sales_data.bronze.raw_orders
```


```
sales_data.gold.monthly_revenue
```
...
```
unity_catalog └─ catalogs ├─ sales_dev │ ├─ raw (schema) │ ├─ staging (schema) │ └─ curated (schema) │ ├─ orders_dev (table) │ └─ customers_dev (table) │ ├─ sales_test │ ├─ raw │ ├─ staging │ └─ curated │ └─ orders_test │ └─ sales_prod ├─ raw ├─ staging └─ curated ├─ orders └─ customers
```
...
```
customer_analytics_shared
```


```
internal_sales_raw_data
```
...
```
abfss://container@storage.dfs.core.windows.net/env/layer/domain/table/
```


```
abfss://datalake@company.dfs.core.windows.net/prod/gold/sales/monthly_revenue/
```
